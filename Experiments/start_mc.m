...
for i = 1:length(Extracted_Feature)
    ...
    for b=1:bound
        ...
        if (expand > num_Dimension)
            num_Transition(expand,expand) = 0;
            num_Dimension = size(num_Transition,1); % expanding total # of Apps
        end
    ...
    for j = 1:num_test
        ...
        if cur_App <= num_Dimension
            [tval User(i).Result(j,1)] = max(User(i).TransitionMatrix(cur_App,:));
        else
    ...
    %predict(tree,X)
    if ispc
        save('.\Result_MC.mat', 'User', 'weighted_acc', 'total_acc');
    else
        save('./Result_MC.mat', 'User', 'weighted_acc', 'total_acc');
    end    
end